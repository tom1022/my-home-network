...
- パスワードや鍵などの機密情報は、Infisical から `lookup('env', ...)` で供給される変数構成にすること。
...
- コマンドの使用はいかなる場合においてもlsやlsblkなどシステムの変更を行わないものに限定し、コード化されていないコマンドの使用は禁止
...
# 6. IPアドレスなどの仕様
- 管理PCなどProxmoxノードや仮想環境同士での接続以外のものは192.168.1.0/24を使用してください
- Proxmoxノードや仮想環境同士での接続は、172.16.0.0/24を使用することを徹底してください
- IPアドレスの割当は基本的にTerraformに定義されるものを使用してください

# 7. チャット内でのルール
- コマンドを出力する場合にはマークダウンのリンクを使わずに、パスはきちんとパスどおりに記述し、コードブロックで囲むこと。
- コマンドを出力する場合には改行をしないこと
- 日本語で回答すること

# 禁止事項
- すでに実装されているロジックの再実装
- ベストプラクティスを無視した一般的ではない独自実装
