...
def _collect_with(tmp: Path, inventory_text: str, extra_host=None, extra_ip=None):
    ...
    entries = mod.parse_terraform_locals(tmp / "locals.tf") + mod.parse_inventory(
        tmp / "inventory.yml"
    )
    if extra_host is not None:
        entries = list(entries) + [
            AddressEntry(extra_host, extra_ip, "fixture", 1, "external_ip")
        ]
    return entries
...
def test_excluded_host_suppresses_mismatch():
    with tempfile.TemporaryDirectory() as d:
        ...
        assert mismatches == [], (
            "excluded host must not be judged even if its values diverge"
        )
