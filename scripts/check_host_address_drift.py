...
STATIC_UNMANAGED_HOSTS: frozenset[str] = frozenset(
    {"mirakurun-epgstation", "nextcloud"}
)
...
def _map_items(node: yaml.Node, path: Path) -> list:
    if not isinstance(node, yaml.MappingNode):
        raise ParseError(
            f"{_rel(path)}: マッピングを期待した位置に {type(node).__name__}"
        )
    return node.value
...
def parse_terraform_locals(path: Path) -> list[AddressEntry]:
    ...
    for lineno, raw in enumerate(text.splitlines(), 1):
        ...
        if m_ip:
            if host is None:
                raise ParseError(
                    f"{_rel(path)}:{lineno}: ip0/ip1 がホストブロックの外に出現"
                )
    ...
    if depth != 0:
        raise ParseError(
            f"{_rel(path)}: 波括弧の対応が取れない (EOF 時点で depth={depth})"
        )
    if not entries:
...
def parse_pbs_backup_targets(path: Path) -> tuple[list[AddressEntry], set[str]]:
    ...
    for item in targets.value:
        ...
        if name_node is None or ip_node is None:
            raise ParseError(
                f"{_rel(path)}:{_line(item)}: name/ip を欠くバックアップ対象"
            )
        entries.append(
            AddressEntry(
                name_node.value,
...
def parse_k3s_group_vars(path: Path) -> list[AddressEntry]:
    ...
    if not m:
        raise ParseError(
            f"{_rel(path)}: argocd_gitea_repo_url から IP を抽出できない: {node.value!r}"
        )
    return [
        AddressEntry(
            "ref:argocd_gitea_repo_url",
            m.group(1),
            _rel(path),
            _line(node),
            "reference",
...
def parse_vps_host_vars(path: Path) -> list[AddressEntry]:
    ...
    for key in _VPS_UPSTREAM_KEYS:
        ...
        if node is None:
            continue
        entries.append(
            AddressEntry(f"ref:{key}", node.value, _rel(path), _line(node), "reference")
        )
    ...
    if tcp_upstreams is not None:
        if not isinstance(tcp_upstreams, yaml.SequenceNode):
            raise ParseError(
                f"{_rel(path)}: vps_proxy_tcp_upstreams はシーケンスを期待"
            )
        for group in tcp_upstreams.value:
            ...
            for member in members.value:
                ...
                if host_node is None:
                    raise ParseError(
                        f"{_rel(path)}:{_line(member)}: members に host が無い"
                    )
                entries.append(
                    AddressEntry(
                        f"ref:tcp_upstream:{host_node.value}",
                        host_node.value,
                        _rel(path),
                        _line(host_node),
                        "reference",
...
def find_mismatches(
    entries: Sequence[AddressEntry], excluded: Optional[frozenset[str]] = None
) -> Sequence[Mismatch]:
    if excluded is None:
