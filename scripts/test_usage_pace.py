...
def test_compute_window_pace_on_target():
    ...
    assert (
        pace["will_stall_before_reset"] is False
    )  # lands exactly at reset, not before
...
def test_compute_window_pace_will_stall_before_reset():
    ...
    pace = mod.compute_window_pace(
        40.0, reset, now - timedelta(hours=1), mod.SESSION_HOURS
    )
...
def test_compute_window_pace_rate_override_used_for_rate_fields_only():
    ...
    smoothed = mod.compute_window_pace(
        20.0, reset, now, mod.SESSION_HOURS, rate_override=5.0
    )
...
def test_compute_pace_propagates_upstream_failure():
    result, history = mod.compute_pace(
        {"ok": False, "error": "boom"}, datetime.now(timezone.utc)
    )
...
def test_compute_pace_warms_up_on_first_sample():
    ...
    usage = {
        ...
        "weeks": [
            {"label": "all models", "percent": 14, "resets": "Sep 5, 3am (Asia/Tokyo)"}
        ],
    }
...
def test_compute_pace_builds_up_history_and_recommends_lanes():
    ...
    usage = {
        ...
        "weeks": [
            {"label": "all models", "percent": 20, "resets": "Sep 5, 3am (Asia/Tokyo)"}
        ],
    }
    ...
    assert (
        result["session"]["will_stall_before_reset"] is True
    )  # running hot, per the scenario
...
def test_compute_pace_missing_all_models_week():
    result, history = mod.compute_pace(
        {
            "ok": True,
            "session": {"percent": 1, "resets": "Sep 2, 7pm (Asia/Tokyo)"},
            "weeks": [],
        },
