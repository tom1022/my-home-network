...
def update_history(
    history: list[dict], sample: dict, max_keep: int = MAX_HISTORY_SAMPLES
) -> list[dict]:
    ...
    if (
        history
        and sample["elapsed_hours"]
        < history[-1]["elapsed_hours"] - ROLLOVER_TOLERANCE_HOURS
    ):
        history = []
...
def compute_window_pace(
    ...
    required_rate = (
        (TARGET_PERCENT - actual_percent) / remaining_h if remaining_h > 0 else None
    )
    ...
    return {
        ...
        "delta": round(delta, 2),
        "current_rate_pt_per_h": round(current_rate, 4)
        if current_rate is not None
        else None,
        "required_rate_pt_per_h": round(required_rate, 4)
...
def compute_pace(
    ...
    if not usage.get("ok"):
        return {
            "ok": False,
            "error": usage.get("error", "upstream usage fetch failed"),
        }, history
    ...
    session = compute_window_pace(
        session_raw["percent"], session_reset, now, SESSION_HOURS, rate_override=rate
    )
    ...
    lanes = {
        "current": current_lanes,
        "recommended": recommend_lanes(current_lanes, session["speedup"]),
    }
...
def fetch_usage() -> dict:
    ...
    try:
        ...
        return {
            "ok": False,
            "error": f"check_claude_usage.py did not respond within {TIMEOUT_SECONDS}s",
        }
    try:
        ...
        return {
            "ok": False,
            "error": f"could not parse check_claude_usage.py output: {e}",
        }
...
def main() -> int:
    ...
    parser.add_argument(
        "--history-file",
        default=None,
        help="path to persist session sample history (JSON array)",
    )
    parser.add_argument(
        "--lanes-file",
        default=None,
        help="path to a text file containing the current lane count",
    )
    parser.add_argument(
        "--lanes",
        type=int,
        default=None,
        help="explicit current lane count, overrides --lanes-file",
    )
    ...
    current_lanes = (
        ...
        else (read_lanes(args.lanes_file) if args.lanes_file else None)
    )

    result, new_history = compute_pace(
        usage, datetime.now(timezone.utc), history, current_lanes
    )
