...
_WEEK_RE = re.compile(r"Current week \(([^)]+)\):\s*(\d+)%\s*used(?:\s*\S\s*resets\s*(.+))?")
...
def parse_usage_text(text: str) -> dict:
    ...
    for line in text.splitlines():
        ...
        if m:
            weeks.append({"label": m.group(1), "percent": int(m.group(2)), "resets": m.group(3)})
...
def fetch_usage() -> dict:
    try:
        ...
        return {"ok": False, "error": f"claude did not respond within {TIMEOUT_SECONDS}s"}
    except FileNotFoundError:
        return {"ok": False, "error": "claude CLI not found on PATH"}

    if proc.returncode != 0:
        return {"ok": False, "error": f"claude exited {proc.returncode}: {proc.stderr.strip()[:500]}"}
