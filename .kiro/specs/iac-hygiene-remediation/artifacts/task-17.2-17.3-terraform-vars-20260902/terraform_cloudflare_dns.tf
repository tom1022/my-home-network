...
locals {
  ...
  vps_hosts = {
    ...
    root = { name = "fickledev.com", proxied = true, aaaa = true, enabled = true }
    # vps_proxy_domain_www; primary vhost, see ansible/roles/vps_proxy/templates/fickledev.com.conf.j2
    www = { name = "www.fickledev.com", proxied = true, aaaa = true, enabled = true }
    ...
    # 再利用するため保持する。
    mail = { name = "mail.fickledev.com", proxied = false, aaaa = true, enabled = true }
    # No corresponding vps_proxy_* upstream found in ansible/inventory/host_vars/vps/main.yml at code time.
    mc = { name = "mc.fickledev.com", proxied = false, aaaa = true, enabled = true }
    # xrayvpn は停止中 (vps_proxy_xray_sni を空にして SNI 分岐も無効化済み)。
    # 復帰時は enabled を true に戻して apply するのみ。
    appflowy = { name = "appflowy.fickledev.com", proxied = false, aaaa = false, enabled = false }
  }
  ...
  tunnel_cnames = {
    console = { name = "console.fickledev.com", tunnel = "guacamole" }
    crafty  = { name = "crafty.fickledev.com", tunnel = "kubernetes" }
    # 認証基盤 (Kanidm) の新規公開ホスト名 (task 26.1)。DNS レコードと
    # トンネルの ingress 経路 (cloudflare_zero_trust.tf) をあわせて Terraform 管理下に置く。
    kanidm = { name = "kanidm.fickledev.com", tunnel = "kubernetes" }
  }
  ...
  dns_records = merge(local.vps_a_records, local.vps_aaaa_records, local.cname_records, local.other_dns_records)
}
