...
**測定日**: 外向きは 2026-09-01 に VPS 上から、内向きは 2026-09-04 に外部経路から、いずれも TCP 接続によって確認した。定義や管理画面の表示ではなく実接続による。内向きについては測定経路の制約が結果の解釈を左右するため、後述する。
...
2026-09-01 の測定では応答の種類が 3 つに分かれていた。

| ポート | 外部からの応答 |
|---|---|
| 143 / 443 / 465 / 587 / 993 | TCP 確立 |
| 110 / 995 | RST |
| 22 / 25 | ドロップ |

**この測定は経路が家庭回線であった疑いがあり、25 番の結果を VPS 側および提供元側の状態の証拠として用いることはできない。**OP25B は outbound port 25 blocking であり、塞がれるのは発信側の外向き 25 番である。家庭回線から VPS の 25 番へ接続を試みた場合、接続は家庭側で落ちるため、**測っているのは家庭回線の外向き 25 番であって VPS の内向き 25 番ではない**。25 番以外は家庭回線を素通りして VPS まで届くので、上表の分かれ方はこの経路だけで説明が付く。「25 番以外のポートが RST を返すのだから上流は汎用的にはパケットを通している」という推論も、発信側で 25 番だけが落ちる経路の下では成立しない。22 番のドロップは VPS への接続を tailnet に限っていることによるものであり、本 spec の対象外である。

OP25B の影響を受けない経路からは、内向き 25 番の到達が一貫して確認されている。2026-09-02 の測定 (10 節) では 5 拠点すべてで TCP 接続が確立し、2026-09-04 の再確認でも 5 ノードのうち 4 ノードで確立した。**内向き 25 番は到達する。提供元のコントロールパネルでの遮断解除は行っていないため、当初から到達していたと考えられる。**同一のホストに対して 2026-09-01 の測定だけが 25 番のドロップを示したことは、経路の差として説明が付く。

**測定経路の制約**: 家庭回線から VPS の 25 番へ接続を試みても、VPS の内向き 25 番は測れない。測っているのは家庭回線の外向き 25 番である。内向き 25 番の確認は必ず OP25B の影響を受けない経路から行う。

**設計への含意**:

- 内向き 25 番は到達しており、構成全体の前提は満たされている。提供元側に解除を要する操作は存在せず、コードによる管理の対象外となる手順にも当たらない。
- 段階 0 の関門としては到達性の確認自体を残す。確認は OP25B の影響を受けない経路からの実接続で行う。
...
| ポート | 拠点 | 到達 | 結果 |
|---|---|---|---|
| 25 | bg1(ブルガリア) / ir4・ir8(イラン) / tr2(トルコ) / us1(米国) | 5/5 | 全拠点でTCP接続確立。**内向き 25 番は到達する** |
...
**結論**:
- 25番ポート: 全拠点から到達する。提供元側に解除を要する操作は存在しない。8 節に記した 2026-09-01 の測定が 25 番のみドロップとなったのは経路が家庭回線であった疑いによるもので、OP25B の対象外である本節の経路では 5 拠点すべてで確立している。同一のホストに対する結果の差は経路の差である。
...
一方、465番と587番についてはMXToolbox等による独立した裏取りは行っておらず、check-host.net単独の結果に依拠している。この差はタスク1.1の後続判断には影響しない。後続タスクの着手判断を左右する関門は25番ポートの到達のみであり、25番は2系統の情報源で到達を確認済みである。465 / 587番の到達確認は「提供元側の追加操作を要しない」ことを補足的に裏付ける記録であって、それ自体が着手判断のブロッカーではない。
...
## タスク8.2: 復元検証 (2026-09-04)

手順書は独立ファイル `.kiro/specs/mail-platform/mailbox-restore-recreate-runbook.md` として作成した (復元手順・再作成手順とも本節ではなくそちらに記述する)。本節は復元検証の方法・実測結果のみを記録する。

### 採った検証方法と安全性の担保

稼働中の `/srv/vmail/` を一切変更せず、実際の書庫 1 世代を使って「使い捨て利用者を作り、その利用者のメールボックスだけを書庫から復元して取得できることを確認する」方式 (プロンプトが提示した選択肢の1つ) を採った。

- Kanidm に使い捨て人物アカウント `mailrestore902verify` を作成し (`mail` 属性設定、`mail_users` 所属)、本人セッションでアプリケーションパスワードを自己発行した (管理者代理発行ではない。タスク3.1/6.3/6.5/8.1と同じ手順)。
- production の submission (587、`mail.fickledev.com`) へこのアプリケーションパスワードで認証し、**自分自身宛にテストメールを送信**した (書庫に実際に取り込ませるため、外部ドメインを騙った未認証メールではなく、認証済みの自ドメイン送信を選んだ。後述のとおり未認証の外部送信はrspamdに拒否された)。
- 送信直後にIMAP (993) で1件取得できることを確認し、`Message-ID`を記録した。
- `systemctl start mail-backup-pull.service` (pbs) を手動起動し、この状態を含む新しい書庫世代 `mailbox-20260903T152720Z.tar.gz` を生成させた。`tar tzf`および該当ファイルの中身の直接確認により、このテストメール (同一の`DKIM-Signature`/`Message-ID`/本文) が実際に書庫へ入ったことを確認した。
- **喪失をシミュレート**: `k3s-agent-minipc`のPVディレクトリ (`/var/lib/rancher/k3s/storage/<pvc-uid>_mailbox_mailbox-data/`) 直下のうち、`mailrestore902verify`のサブディレクトリ**だけ**を`rm -rf`した (ansibleのbecome、ホストのファイルシステムを直接操作。他の10件の既存残骸ディレクトリ、および他の一切の稼働中データには触れていない。`kubectl delete pv`/`pvc`は使用していない)。IMAP再SELECTで0件になることを確認し、喪失が成立したことを実測した。
- **復元**: pbs上で書庫からこの利用者のサブツリーだけを取り出して単独tarに再パックし、ansibleの`fetch`/`copy`でローカル経由`k3s-agent-minipc`へ転送、対象ディレクトリへ展開した (`chown -R 1000:1000`、ディレクトリを`chmod 2700`で復元)。書庫取得専用の制限鍵 (`MAIL_BACKUP_SSH_PRIVATE_KEY`、forced command付き) は使わず、運用者の通常のansible管理アクセスのみで完結させた。
- 復元後、同じ利用者・同じアプリケーションパスワードでIMAP再ログインし、`SELECT`が1件を返すこと、`Message-ID`が送信時と一致すること、`FETCH`した本文がバイト単位で一致することを確認した。**合格**。

検証に使ったテストメールは、書庫を取得させる**前**に送信し、その後に手動起動した書庫取得で初めて取り込ませたものである (プロンプトが要求する順序どおり)。書庫投入後に追加送信したメールではなく、書庫に実際に入っているメールで復元を確認した。

### 想定外の事象

1. **未認証の外部送信はrspamdに拒否された**: 当初、送信元を`gmail.com`と騙った未認証SMTP (25番) でテストメールを投入しようとしたが、`554 5.7.1 Spam message rejected`で拒否された。rspamdログのスコア内訳は`[17.50/11.00] HFILTER_HOSTNAME_UNKNOWN(6.00), VIOLATED_DIRECT_SPF(3.50), R_SPF_SOFTFAIL(2.50), RDNS_NONE(2.00), DMARC_POLICY_SOFTFAIL(1.50), HFILTER_HELO_IP_A(1.00), R_DKIM_NA(1.00)`。送信元 (sandbox環境のTailscale/実インターネットIP) にPTRが無くgmail.comのSPFにも一致しないため、実在ドメインを騙った未認証送信では正当に拒否される。任意の実在ドメインで再試行を重ねるより、**submission (587) を認証済みで使い、使い捨て利用者から自分自身へ送る**方式に切り替えた方が本来のメール基盤の使われ方に近く、迷惑メール判定を回避する不自然な細工も不要になるため、この方式へ変更した。
2. **IMAPログインだけで空のメールボックスが自動的に再作成される**: `rm -rf`で喪失をシミュレートした直後にIMAPで存在確認を行ったところ、Dovecotが新しい`uidvalidity`で空のMaildirディレクトリを自動生成していた (userdb staticの既定動作)。この状態のまま書庫を展開すると、書庫が持つ古い`uidvalidity`と衝突する。**復元前にこの自動生成物を再度`rm -rf`してから展開する**必要があることを実機で発見し、runbookに明記した。
3. **タイマーのNEXT表示が起動前後でジッターした**: 手動起動前は`NEXT: 2026-09-04 04:10:58 UTC`、起動直後の確認では`04:16:07 UTC`だった。ただし`LAST`/`PASSED`(直近のタイマー起動実績) はいずれも起動前後で不変 (`Thu 2026-09-03 15:00:04 UTC`のまま) であり、その後複数回再クエリしても`04:16:07 UTC`のまま安定した。`RandomizedDelaySec=10m`の範囲内でのジッターであり、対象のカレンダー当日 (2026-09-04 04時台) 自体は変わっていない。タスク8.1が確認した「手動起動はスケジュールに影響しない」という結論と大枠は一致するが、`NEXT`表示の秒単位の値は再クエリのたびに多少ジッターしうる点は本タスクで新たに気づいた事実として記録する。手動起動 (`systemctl start`) は`.timer`ではなく`.service`を直接起動しており、`LAST`/`PASSED`はタイマー自身の発火実績のみを指すため、この経路そのものが定期実行のトリガー履歴を書き換えることはない。

### 使い捨てアカウントの後始末

検証後、`mailrestore902verify`を`mail_users`から除外 (`PUT .../_attr/member`に`[]`、除外後`GET`が空であることを確認)、`DELETE /v1/person/mailrestore902verify`で削除 (削除後の`GET`が`null`を返すことを確認)。`k3s-agent-minipc`側の`mailrestore902verify`ディレクトリも (host権限で作成・削除が可能だったため) 削除し、他の10件の既存残骸ディレクトリのみが残る元の状態に戻した。`pbs`側の一時ファイル (`/tmp/restore-verify/`) も削除した。`terraform-kanidm`で`infisical run --env=prod -- terraform plan -detailed-exitcode`を実行し、**"No changes."** (exit code 0) を確認した。手動起動で生成した書庫世代 (`mailbox-20260903T152720Z.tar.gz`) はそのまま残置している (通常の世代の1つとして扱い、上限14世代のローテーションに委ねる。検証専用の内容を含まない実データそのものであるため削除不要と判断した)。

### 完了可否

**完了。** 書庫からの復元手順を独立文書として記述し、実際に書庫へ取り込ませたメールを使って復元・IMAP取得の成立を実機で確認した (手順の記述のみに留めていない)。DKIMの鍵およびKanidm上のメール用定義の再作成手順も同文書に記述した (実行はしていない。要件が求めるのは記述のみ)。段階4 (MX公開) の開始条件である「復元の成立が確認済み」の状態を満たしたと判断する。
