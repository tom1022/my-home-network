...
├── ansible/
│   ├── roles/
│   │   ├── mail_backup/             # メールボックス書庫の取り出しと保存 (取得元と保存先の 2 面)
│   │   ├── systemd_failure_notify/  # 定期実行の失敗を独立経路へ通知する雛形ユニット
...
│   │       │   ├── rspamd-ratelimit.conf.j2 # 認証済み送信の量的上限
│   │       │   └── ldap-*.cf.j2             # 宛先照合の検索定義
│   │       └── handlers/main.yml
│   └── playbooks/{mailserver.yml,mail_backup.yml}
├── terraform/
...
- 認証済みの送信に、送信者単位の量的な上限を持つ。アプリケーションパスワードは有効期限を設定できないため、資格情報が漏れた場合に「認証済みで署名も方針も正当」な大量送信が成立する。検査だけでは量を止められない。上限は個人利用の送信を妨げない値とし、超過は一時的な失敗として扱う。恒久的な拒否にすると上限に当たった正当な送信が再送されずに失われる。
- 量的な上限を受信側の経路に持ち込まない。認証を伴わない経路に上限を置くと、正当な受信の突発が遅延する。上限は認証済みの送信者を鍵とするものに限る。
- 障害通知および配送不能通知の宛先を、量的な上限の対象から除外する。
...
- Integration: 送信の量的な上限は迷惑メール判定機構の速度制限機能で与える。docker-mailserver には当該機構の追記用ディレクトリへ差し込む経路が存在しないため、置換用ディレクトリを用いる。置換はセクション単位であるため、上限の定義と除外する宛先を 1 つのファイルに揃えて宣言する。ウイルス検査の接続先と同じ経路である。
- Integration: 上限の値は、送信者単位の 2 本の容器として持つ。短期は容量 30・毎時 30、日次は容量 50・毎日 100 とする。単位は通数ではなく宛先数であり、容器の加算値は 1 通あたりの受取人数になる。1 通あたりの宛先数に対する上限を別に置かないのはこのためで、多数の宛先を持つ 1 通は同じ容器をその宛先数だけ消費する。
- Integration: 値の根拠は、実績からの外挿ではなく人間の作業速度と侵害時の規模の間に置くことにある。記録上の送信実績は外挿できるだけの量がない。毎時 30 宛先は、手作業の送信では到達しない一方、漏れた資格情報による送信 (毎時数百から数千の規模) に対しては 2 桁の抑制になる。毎日 100 宛先は、個人の 1 日の送信 (多くて十数通) に対して 1 桁の余裕を持ちながら、送信元の評価が失われる規模には届かない量に抑える。
...
- Validation: 送信の量的な上限は、設定の反映と発火の双方を確認する。反映は稼働中の設定の書き出しが宣言した値と一致することで、発火は上限を一時的に極小の値へ置いた状態で超過が一時的な失敗になることで確認する。あわせて本来の値において、通常の 1 通が上限に掛からないことを確認する。
...
- 認証は宣言した passdb だけが担う。コンテナイメージの既定の設定は、環境変数由来の資格情報を任意の利用者名に対して受理する passdb を、追加設定より前に宣言している。同名の宣言を追加設定で再宣言し、資格情報を crypt(3) が生成しえない値へ上書きして到達不能にする。上書きを欠いた状態では、認証基盤に存在しない利用者名と既定の資格情報で取得の認証が成立する。
...
- Integration: 追加設定はイメージ既定の設定の末尾から読み込まれ、同名の宣言は 1 つに結合されて後から読んだ値が優先される。既定の宣言そのものを除去する手段は追加設定の側に無いため、資格情報の上書きによって到達不能にする。VPS 側が既定の項目定義ファイルを差し替えるのと同じく、イメージ既定を無害化する処置である。
- Validation: 配送の受け口へ引き渡したメールが永続ボリュームに現れること、取得がアプリケーションパスワードで成立すること、複製を再作成してもメールが失われないこと。あわせて、`doveconf -P` が出力する passdb の集合を確認し、認証基盤に存在しない利用者名では取得の認証がいかなる資格情報でも成立しないことを、公開している取得の待受に対する実際の IMAP ログインで確認する。
- Risks: 永続ボリュームの喪失は蓄積したメールの喪失にあたる。復旧対象とする方針に基づき、保全の成果物の存在を段階の完了条件に含める。
- Risks: イメージ既定の passdb を無害化する処置は、既定の宣言の名称に一致させることで成立する。主要版の更新で名称が変われば上書きが外れ、既定の資格情報による認証が復活する。イメージをダイジェストで固定し、更新の際に passdb の出力を確認する。
...
- 取得に失敗した場合、失敗を記録に残し成功したものとして扱わない。あわせて、メール基盤とは独立した経路で失敗を通知する。記録に残すことは気づけることを意味しない。定期実行は成功時に何も出力しないため、通知が無ければ取得が止まったことは最新の書庫が古くなり続ける形でしか現れない。
- 通知の経路にメールを用いない。メール基盤の障害を通知する経路がメール基盤に依存すると、通知を最も要する場面で届かない。
- 通知の内容は、どのホストのどのユニットがいつどう失敗したかまでとする。資格情報および個人のアカウント名を含めない。詳細はホスト上の記録を参照する前提とする。
...
- Integration: 失敗の通知は、定期実行の仕組みが備える失敗時の起動指定によって行う。取得のユニットは失敗したユニット名を渡すだけで、通知の実装を自身に持たない。ハンドラはユニット名を実体名として受け取る雛形のユニットとして持ち、保全に固有のものにしない。同じ宣言を 1 行足すだけで、フリート上の他の定期実行も同じ経路で通知できる。
- Integration: 通知先の宛先情報はリポジトリに置かず、適用時に秘密情報の管理基盤から取得してホスト上のファイルへ配る。当該ファイルは所有者を root、権限を 0600 とし、ハンドラのユニットが環境変数の供給元として読む。要求の発行時に引数へ置かない。引数に置くと、要求が飛んでいる間ホスト上の任意の利用者から見える。
- Integration: 通知そのものが届かなかった場合、ハンドラ自身を失敗として終える。通知の不達が記録上「通知した」と区別できなくなることを避ける。
- Validation: 書庫が生成されること、世代数の上限が機能すること、書庫から復元したメールボックスが取得できること。取得を意図的に失敗させ、通知が実際に届くこと。確認の後、失敗させた状態を戻し、通常の取得が成功すること。
...
**保全の失敗**: 書庫の取得に失敗した場合、失敗を記録に残し成功したものとして扱わない。あわせて、メール基盤とは独立した経路で失敗を通知する。通知が届かなかった場合は通知の処理自体を失敗として終え、成功と区別できる状態にする。

### Monitoring

メールボックスの容量を確認できる状態にする。配送の待ち行列の滞留と保全の失敗を記録として残す。本 spec は監視基盤を導入しない。

定期実行の失敗については、記録の確認を待たずに通知する。通知はメール基盤とは独立した経路で行い、通知の本文は種別の識別子で始めることで、同じ経路を使う他の用途と取り違えられない形にする。通知が扱うのはユニットが失敗として終わった場合に限る。取得が成功しつつ内容が空である場合のような、失敗として現れない異常はこの経路では検知できない。
