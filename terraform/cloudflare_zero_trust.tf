...
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "kubernetes" {
  ...
  config = {
    ingress = [
      {
        hostname = "idp.fickledev.com"
        service  = "http://authentik.authentik-fickledev.svc.cluster.local:80"
        origin_request = {
          http_host_header = "idp.fickledev.com"
        }
      },
      {
